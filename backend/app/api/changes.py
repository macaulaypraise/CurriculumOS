...
async def approve_curriculum_change(project_id: int, request: ChangeApprovalRequest, session: AsyncSession = Depends(get_db)) -> dict[str, Any]:
    ...
    try:
        ...
        await session.rollback(); course = await load_project_course(session, project_id)
        if course is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Project has no course")
    try:
        await session.flush(); refreshed = await load_project_course(session, project_id)
        ...
        await session.rollback(); fallback_course = await load_project_course(session, project_id); fallback_graph = serialize_course_graph(fallback_course) if fallback_course else {"id": course.id, "modules": []}
