...
export function ModuleNode({ data, selected }: NodeProps<ModuleNodeData>) {
  ...
  return (
    <div className={`relative min-w-[220px] max-w-[260px] cursor-pointer rounded-[15px] border border-zinc-800 border-l-2 ${statusStrip[status]} bg-zinc-900 p-4 shadow-lg shadow-black/20 transition-all duration-200 ease-out hover:-translate-y-0.5 hover:border-zinc-600 hover:shadow-xl hover:shadow-black/35 ${data.dimmed ? 'scale-[0.98] opacity-25' : 'opacity-100'} ${selected || data.focused ? 'animate-[pulse_2s_ease-out_infinite] border-zinc-600 shadow-cyan-500/10' : ''} ${data.updated ? 'shadow-[0_0_15px_rgba(6,182,212,0.5)] ring-1 ring-cyan-400/60' : ''}`}>
      ...
      <div className="space-y-2"><h3 className="truncate text-sm font-semibold text-zinc-100">{title}</h3><p className="truncate text-xs text-zinc-400">{data.description ?? 'Structured learning sequence and prerequisite relationships.'}</p></div>
      <div className="mt-3 flex items-center gap-1.5 text-[10px] text-zinc-500"><span>📚 {data.lessonCount ?? 0} Lessons</span><span>•</span><span>🎯 {data.outcomeCount ?? 0} Outcomes</span></div>
